...
def export(
    ...
    *,
    ...
    Acceptable types of inputs (for `args` and `kwargs`) and outputs include:
        - Primitive types, i.e. torch.Tensor, int, float, bool.
    ...
    assumptions hold true. These assumptions are validated during tracing
    according to shapes (not values) of input tensors, values of other type
    inputs that are specialzied and `constraints` arguments. If any assumption
    can not be validated, a fatal error will be raised. When that happens,
    the error message will include suggested code needed to construct necessary
    constraints to meet the assumptions, e.g.
        ```
